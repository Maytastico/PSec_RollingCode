...
void setup() {
  ...
  pinMode(BTN_TASK1, INPUT);
  pinMode(BTN_TASK2, INPUT);
...
void loop() {
  ...
  if (digitalRead(BTN_TASK2) == HIGH) {
    log_i("Button 1 pressed, sending encrypted rolling code...");
  ...
  }else if(digitalRead(BTN_TASK1) == HIGH){
    log_i("Button 2 pressed, sending a test message...");
    // Einen sauberen 16-Byte Sendepuffer anlegen
    uint8_t txBuffer[16];
    memset(txBuffer, 0, sizeof(txBuffer));

    // RGB-LED blau blinken lassen als optisches Feedback für das Senden
    ui.flashLedColor(2, 0, 100, 100, 0); 
